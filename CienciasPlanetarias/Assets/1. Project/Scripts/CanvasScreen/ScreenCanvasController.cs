...
public class ScreenCanvasController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Carrega o appConfig do JSON
        string configPath = System.IO.Path.Combine(Application.streamingAssetsPath, "appconfig.json");
        if (System.IO.File.Exists(configPath))
        {
            string json = System.IO.File.ReadAllText(configPath);
            appConfig = JsonUtility.FromJson<RealGames.AppConfig>(json);
        }
        else
        {
            Debug.LogError($"Arquivo de configuração não encontrado: {configPath}");
        }
    ...
    public void GoBack()
    {
        ScreenManager.SetCallScreen(previusScreen);
    }
